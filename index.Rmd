...
P1<-ggplot(new_age,aes(x=Year,y=U.p,col=Group))+geom_smooth(se=FALSE)+theme+ggtitle("The unemployment across age group ")
...
P2 <- ggplot(age_graph_CAMINY,aes(x=Year,y=U.p,col=Group)) + geom_smooth(se=FALSE) +facet_wrap(~label)+ggtitle("The age unemployment rate across region")+ theme +scale_x_continuous(breaks=c(2003,2005,2007,2009,2011,2013))+scale_y_continuous(breaks=c(4,8,12,16,20,24))+ geom_vline(xintercept = 2008)+ theme(legend.position="none")

#--------------------------

colnames(Sex.Age)[colnames(Sex.Age)=="Race"] <- "Sex"

P3 <- Sex.Age %>% ggplot(aes(x=Year,y=U.p,col=Age_Group))+geom_smooth(se=FALSE)+facet_grid(~Sex)+ theme+ theme(legend.position="none")+ggtitle("The age unemployment rate across sex")

#------------------------

all_races <- Demographics %>% filter(Group == "Men" | Group == "Women")

P4 <-  ggplot(all_races,aes(x = Year, y = U.p, col = Group)) + geom_smooth(se = FALSE) + ylab("Unemployment Percent") + ggtitle("Gender Unemployment across all States") + theme  + geom_vline(xintercept = 2008) + scale_x_continuous(breaks = c(2003, 2005, 2007, 2009, 2011, 2013))
...
P5 <-  ggplot(gender_across_races_facet,aes(x = Year, y = U.p, col = Group)) + geom_smooth( se = FALSE) + ylab("Unemployment Percent") + ggtitle("Gender Unemployment across Races")  + theme  + geom_vline(xintercept = 2008) + scale_x_continuous(breaks = c(2003, 2005, 2007, 2009, 2011, 2013))
...
P6 <-  ggplot(gender_across_region_facet,aes(x = Year, y = U.p, col = Group)) + geom_smooth(se=FALSE) + ylab("Unemployment Percent") + ggtitle("Gender Unemployment across Regions") + facet_wrap(~ label) + scale_x_continuous(breaks = c(2003, 2005, 2007, 2009, 2011, 2013)) + geom_vline(xintercept = 2008) + theme
...
P7 <- ggplot(New_Ethnicity,aes(x=Year,y=U.p))+geom_smooth(aes(col=Group),se=FALSE)+ ggtitle("Unemployment and Ethnicity in the US")+geom_vline(xintercept = 2008)+ylab("Unemployment Rate, %") + xlab("Year") + scale_x_continuous(breaks=c(2003,2005,2007,2009,2011,2013))+scale_y_continuous(breaks=c(4,8,12,16,20,24))+ geom_vline(xintercept = 2008)+theme
#-----------------

P8<- New_Ethnicity %>% filter(label=="New England"|label=="Pacific"|label=="Midwest"|label=="Midwest North") %>%

  ggplot(aes(x=Year,y=U.p))+geom_smooth(aes(col=Group),se=FALSE)+facet_wrap(~label)+ggtitle("Ethnicity Unemployment across 4 Regions") + geom_vline(xintercept = 2008)+ylab("Unemployment Rate, %") + xlab("Year") + scale_x_continuous(breaks=c(2003,2005,2007,2009,2011,2013))+scale_y_continuous(breaks=c(4,8,12,16,20,24))+ geom_vline(xintercept = 2008)+theme
#---------------
...
Constr.ResBuilding = function(data){
  industry.plotter(data, indus1= "Construction", indus2= "Residential building construction", indus3= "Total, all industries", title = "Construction, Residential Building Construction")
} 
...
#-------------
plot_grid(P10,P11,P12,P13,P14,ncol=3,nrow=2)
