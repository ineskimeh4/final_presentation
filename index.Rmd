...
map6 = State_map %>% inner_join(data %>% filter(Industry=="Manufacturing"|Industry=="Construction"), by=c("region"="State"))
map6 %>% filter(Year==2010|Year==2009|Year==2014|Year==2013) %>% 
  ...
  ggtitle("Manufacturing Vs. Construction Recovery")+ 
  ...
  panel.background = element_rect(fill = "honeydew2"), strip.background = element_rect(fill = "gray95", size = 2),plot.title = element_text(size=48),axis.title=element_text(size=36),axis.text=element_text(size=36),legend.title=element_text(size=36),legend.text=element_text(size=24))+theme(strip.text= element_text(size = 36))
